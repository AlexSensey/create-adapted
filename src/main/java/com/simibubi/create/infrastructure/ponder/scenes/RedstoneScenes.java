...
public class RedstoneScenes {
	...
	public static void nixieTube(SceneBuilder scene, SceneBuildingUtil util) {
		...
		for (int i = 0; i < 3; i++) {
			...
			scene.world().modifyBlockEntityNBT(util.select().position(3 - i, 1, 3), NixieTubeBlockEntity.class, nbt -> {
				...
				nbt.putString("CustomText", CreateSceneBuilder.serializeComponent(scene.world().getHolderLookupProvider(), component));
	...
	public static void redstoneLink(SceneBuilder scene, SceneBuildingUtil util) {
		...
		scene.world().modifyBlockEntityNBT(link1Select, RedstoneLinkBlockEntity.class,
			nbt -> nbt.put("FrequencyLast", CreateSceneBuilder.serializeItemStack(scene.world().getHolderLookupProvider(), iron)));
		scene.idle(7);
		scene.world().modifyBlockEntityNBT(link1Select, RedstoneLinkBlockEntity.class,
			nbt -> nbt.put("FrequencyFirst", CreateSceneBuilder.serializeItemStack(scene.world().getHolderLookupProvider(), sapling)));
		scene.idle(20);
		...
		scene.world().modifyBlockEntityNBT(link3Select, RedstoneLinkBlockEntity.class,
			nbt -> nbt.put("FrequencyLast", CreateSceneBuilder.serializeItemStack(scene.world().getHolderLookupProvider(), gold)));
		scene.idle(7);
		scene.world().modifyBlockEntityNBT(link3Select, RedstoneLinkBlockEntity.class,
			nbt -> nbt.put("FrequencyFirst", CreateSceneBuilder.serializeItemStack(scene.world().getHolderLookupProvider(), sapling)));
		scene.idle(20);
