...
public class RemapHelper {
	private static final RemapMap reMap = new RemapMap();
	...
	static {
		...
		reMap.put("copper_block", Identifier.withDefaultNamespace("copper_block"));
	...
	public static void remap(RegisterEvent event) {
		...
		if (registry == Registries.BLOCK || registry == Registries.ITEM) {
			reMap.forEach((string, Identifier) -> registry.addAlias(asResource(string), Identifier));
		}
	...
	private static class RemapMap extends HashMap<String, Identifier> {
		public Identifier put(String key, Identifier value) {
