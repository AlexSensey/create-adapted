...
public class CreateRegistrateTags {
	...
	private static final Block[] SHULKER_BOXES = Stream.concat(Stream.of(Blocks.SHULKER_BOX),
		Blocks.DYED_SHULKER_BOX.asList().stream()).toArray(Block[]::new);
	...
	private static void genItemTags(RegistrateTagsProvider<Item> provIn) {
		if (provIn instanceof RegistrateItemTagsProvider itemTags) {
			// These families are tagged on their blocks. The legacy specialized
			// provider copied them into the matching item tags as part of generation.
			itemTags.copy(BlockTags.DOORS, BlockItemTags.DOORS.item());
			itemTags.copy(BlockTags.SLABS, BlockItemTags.SLABS.item());
			itemTags.copy(BlockTags.STAIRS, BlockItemTags.STAIRS.item());
			itemTags.copy(BlockTags.TRAPDOORS, BlockItemTags.TRAPDOORS.item());
			itemTags.copy(BlockTags.WALLS, BlockItemTags.WALLS.item());
		}

		CreateTagsProvider<Item> paletteTags = new CreateTagsProvider<>(provIn, Item::builtInRegistryHolder);
		for (AllPaletteStoneTypes palette : AllPaletteStoneTypes.values())
			paletteTags.tag(palette.materialTag)
				.add(palette.baseBlock.get().asItem());
