...
public class OpenCreateMenuButton extends Button {
	private static final Identifier GOGGLES_TEXTURE =
		Identifier.fromNamespaceAndPath(Create.ID, "textures/item/goggles.png");
	...
	protected void extractContents(GuiGraphicsExtractor graphics, int mouseX, int mouseY, float partialTick) {
		graphics.blit(RenderPipelines.GUI_TEXTURED, GOGGLES_TEXTURE, getX() + 2, getY() + 2,
			0, 0, 16, 16, 16, 16);
	}
	...
	public static class OpenConfigButtonHandler {
		...
		public static void onGuiInit(ScreenEvent.Init.Post event) {
			...
			List<AbstractWidget> widgets = event.getListenersList()
				.stream()
			...
			widgets.stream()
				...
				.findFirst()
				.ifPresent(w -> {
					int buttonX = w.getX() + offsetX_ + (onLeft ? -20 : w.getWidth());
					if (screen instanceof PauseScreen) {
						// Modern pause menus turn feedback/report actions into a tightly packed icon row.
						// The old fixed offset landed on a neighbouring button, so its click opened that action.
						int rowY = w.getY();
						if (onLeft)
							buttonX = widgets.stream()
								.filter(widget -> widget.getY() == rowY)
							...
							buttonX = widgets.stream()
								.filter(widget -> widget.getY() == rowY)
								.mapToInt(widget -> widget.getX() + widget.getWidth())
								.max()
								.orElse(w.getX() + w.getWidth()) + 4;
					}
					toAdd.setValue(new OpenCreateMenuButton(buttonX, w.getY()));
				});
