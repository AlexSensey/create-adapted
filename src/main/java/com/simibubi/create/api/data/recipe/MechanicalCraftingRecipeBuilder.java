...
public class MechanicalCraftingRecipeBuilder {
	...
	public MechanicalCraftingRecipeBuilder withRegistries(HolderLookup.Provider registries) {
	...
	public MechanicalCraftingRecipeBuilder key(Character c, TagKey<Item> tag) {
		return this.key(c, Ingredient.of(registries.lookupOrThrow(Registries.ITEM).getOrThrow(tag)));
	}
	...
	public void build(RecipeOutput output, String id) {
		Identifier Identifier = RegisteredObjectsHelper.getKeyOrThrow(this.result);
		Identifier idRs = Identifier.parse(id);
		if (idRs.equals(Identifier)) {
