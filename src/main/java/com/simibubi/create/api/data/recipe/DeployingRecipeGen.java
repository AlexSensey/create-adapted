...
public abstract class DeployingRecipeGen extends ProcessingRecipeGen<ItemApplicationRecipeParams, DeployerApplicationRecipe, ItemApplicationRecipe.Builder<DeployerApplicationRecipe>> {
	...
	/** Minecraft 26.2 stores each vanilla copper family in one collection. */
	public GeneratedRecipe oxidizationChain(WeatheringCopperCollection<? extends ItemLike> collection) {
		var weathering = collection.weathering();
		var waxed = collection.waxed();
		return oxidizationChain(
			List.of(weathering::unaffected, weathering::exposed, weathering::weathered, weathering::oxidized),
			List.of(waxed::unaffected, waxed::exposed, waxed::weathered, waxed::oxidized));
	}
	...
	protected DeployingRecipeGen(HolderLookup.Provider registries, net.minecraft.data.recipes.RecipeOutput output, String defaultNamespace) {
