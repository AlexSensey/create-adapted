...
public enum AllRecipeTypes implements IRecipeTypeInfo, StringRepresentable {
	...
	MECHANICAL_CRAFTING(() -> new MechanicalCraftingRecipe.Serializer().asRecipeSerializer()),
	SEQUENCED_ASSEMBLY(() -> new SequencedAssemblyRecipeSerializer().asRecipeSerializer()),

	TOOLBOX_DYEING(() -> customRecipeSerializer(ToolboxDyeingRecipe::new), () -> RecipeType.CRAFTING, false),
	ITEM_COPYING(() -> customRecipeSerializer(ItemCopyingRecipe::new), () -> RecipeType.CRAFTING, false);
	...
	AllRecipeTypes(StandardProcessingRecipe.Factory<?> processingFactory) {
		this(() -> new Serializer<>(processingFactory).asRecipeSerializer());
	...
	AllRecipeTypes(ProcessingRecipe.Factory<ItemApplicationRecipeParams, ? extends ItemApplicationRecipe> itemApplicationFactory) {
		this(() -> new ItemApplicationRecipe.Serializer<>(itemApplicationFactory).asRecipeSerializer());
		isProcessingRecipe = true;
	}

	private static <T extends CustomRecipe> RecipeSerializer<T> customRecipeSerializer(Function<CraftingBookCategory, T> factory) {
		return new RecipeSerializer<>(
			MapCodec.unit(() -> factory.apply(CraftingBookCategory.MISC)),
			StreamCodec.of((RegistryFriendlyByteBuf buffer, T recipe) -> {}, buffer -> factory.apply(CraftingBookCategory.MISC))
		);
	}
	...
	public <I extends RecipeInput, R extends Recipe<I>> Optional<RecipeHolder<R>> find(I inv, Level world) {
		if (!(world instanceof ServerLevel serverLevel))
			return CreateRecipeClientCache.find(getType(), inv, world);
