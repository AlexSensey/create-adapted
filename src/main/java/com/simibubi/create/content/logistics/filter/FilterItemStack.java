...
public class FilterItemStack {
	...
	public static FilterItemStack of(HolderLookup.Provider registries, CompoundTag tag) {
		...
		ItemStack directStack = ItemStack.OPTIONAL_CODEC.decode(registries.createSerializationContext(NbtOps.INSTANCE), tag)
			.result()
			.map(result -> result.getFirst())
			.orElse(ItemStack.EMPTY);
		if (!directStack.isEmpty())
			return of(directStack);
		...
		ItemStack stack = ItemStack.OPTIONAL_CODEC.decode(registries.createSerializationContext(NbtOps.INSTANCE), stackTag)
			.resultOrPartial(error -> Create.LOGGER.error("Failed to deserialize filter item stack: {}", error))
			.map(result -> result.getFirst())
			.orElse(ItemStack.EMPTY);
		return of(stack);
	}
	...
	public CompoundTag serializeNBT(HolderLookup.Provider registries) {
		if (registries == null || filterItemStack.isEmpty())
			return new CompoundTag();

		return ItemStack.OPTIONAL_CODEC.encodeStart(registries.createSerializationContext(NbtOps.INSTANCE), filterItemStack)
			.resultOrPartial(error -> Create.LOGGER.error("Failed to serialize filter item stack: {}", error))
