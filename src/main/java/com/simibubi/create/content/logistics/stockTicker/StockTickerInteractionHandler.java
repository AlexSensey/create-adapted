...
public class StockTickerInteractionHandler {
	...
	private static void interactWithShop(Player player, Level level, BlockPos targetPos, ItemStack mainHandItem) {
		...
		if (!tickerBE.behaviour.freqId.equals(list.shopNetwork())) {
			...
			player.sendSystemMessage(CreateLang.translate("stock_keeper.wrong_network")
				.style(ChatFormatting.RED)
				.component());
			return;
		}
		...
		for (BigItemStack entry : order.stacks()) {
			...
			AllSoundEvents.DENY.playOnServer(level, player.blockPosition());
			player.sendSystemMessage(CreateLang.translate("stock_keeper.stock_level_too_low")
				.style(ChatFormatting.RED)
				.component());
			return;
		}
		...
		if (occupiedSlots > 0) {
			AllSoundEvents.DENY.playOnServer(level, player.blockPosition());
			player.sendSystemMessage(CreateLang.translate("stock_keeper.cash_register_full")
				.style(ChatFormatting.RED)
				.component());
			return;
		}
		...
		for (boolean simulate : Iterate.trueAndFalse) {
			...
			if (simulate && tally.getTotalCount() != 0) {
				AllSoundEvents.DENY.playOnServer(level, player.blockPosition());
				player.sendSystemMessage(CreateLang.translate("stock_keeper.too_broke")
					.style(ChatFormatting.RED)
					.component());
				return;
			}
