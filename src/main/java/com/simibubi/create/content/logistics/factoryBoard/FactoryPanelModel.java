...
public class FactoryPanelModel extends BakedModelWrapperWithData {
	...
	public void addPanel(List<BakedQuad> quads, BlockState state, PanelSlot slot, PanelType type, PanelState panelState,
		...
		BakedModel model = (BakedModel) factoryPanel.get();
		List<BakedQuad> quadsToAdd = model.getQuads(state, null, rand, data, renderType);
		...
		for (BakedQuad bakedQuad : quadsToAdd) {
			...
			Vec3 quadNormal = Vec3.atLowerCornerOf(bakedQuad.getDirection().getUnitVec3i());
			...
			for (int i = 0; i < vertices.length / BakedQuadHelper.VERTEX_STRIDE; i++) {
				...
				BakedQuadHelper.setNormalXYZ(transformedVertices, i, normal);
			}

			Direction newNormal = Direction.getNearest((int) Math.round(quadNormal.x), (int) Math.round(quadNormal.y),
				(int) Math.round(quadNormal.z), Direction.UP);
