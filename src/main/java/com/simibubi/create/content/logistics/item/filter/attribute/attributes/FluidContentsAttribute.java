...
public record FluidContentsAttribute(@Nullable Fluid fluid) implements ItemAttribute {
	...
	private static List<Fluid> extractFluids(ItemStack stack) {
		...
		for (int tank = 0; tank < capability.size(); tank++) {
			FluidResource resource = capability.getResource(tank);
			if (!resource.isEmpty() && capability.getAmountAsLong(tank) > 0
				&& !fluids.contains(resource.getFluid()))
				fluids.add(resource.getFluid());
		}
		return fluids;
	}
