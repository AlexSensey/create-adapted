...
public class SharedDepotBlockMethods {
	...
	public static ItemInteractionResult onUse(ItemStack stack, BlockState state, Level level, BlockPos pos, Player player,
		...
		if (!wasEmptyHanded && !shouldntPlaceItem) {
			TransportedItemStack transported = new TransportedItemStack(stack.copy());
	...
	public static void onLanded(BlockGetter worldIn, Entity entityIn) {
		...
		BlockPos pos = findDepotInputPosition(worldIn, entityIn);
		if (pos == null)
			return;
	...
	private static BlockPos findDepotInputPosition(BlockGetter worldIn, Entity entityIn) {
		BlockPos onPos = entityIn.getOnPosLegacy();
		if (BlockEntityBehaviour.get(worldIn, onPos, DirectBeltInputBehaviour.TYPE) != null)
			return onPos;

		BlockPos entityPos = entityIn.blockPosition();
		if (BlockEntityBehaviour.get(worldIn, entityPos, DirectBeltInputBehaviour.TYPE) != null)
			return entityPos;

		BlockPos below = entityPos.below();
		if (BlockEntityBehaviour.get(worldIn, below, DirectBeltInputBehaviour.TYPE) != null)
			return below;
