...
public class PipeAttachmentModel extends BakedModelWrapperWithData {
	...
	public ChunkRenderTypeSet getRenderTypes(@NotNull BlockState state, @NotNull RandomSource rand,
		...
		sets.add(model(AllPartialModels.FLUID_PIPE_CASING.get())
			.getRenderTypes(state, rand, data));

		for (Direction d : Iterate.directions)
			for (ComponentPartials partial : ComponentPartials.values())
				sets.add(model(AllPartialModels.PIPE_ATTACHMENTS.get(partial)
					.get(d)
					.get())
					.getRenderTypes(state, rand, data));
	...
	private static void addQuads(List<BakedQuad> quads, BlockState state, Direction side, RandomSource rand,
		ModelData data, PipeModelData pipeData, RenderType renderType) {
		...
		for (Direction d : Iterate.directions) {
			...
			for (ComponentPartials partial : type.partials)
				quads.addAll(model(AllPartialModels.PIPE_ATTACHMENTS.get(partial)
					.get(d)
					.get())
		...
		if (pipeData.isEncased())
			quads.addAll(model(AllPartialModels.FLUID_PIPE_CASING.get())
				.getQuads(state, side, rand, data, renderType));
	}

	private static BakedModel model(Object partial) {
		return (BakedModel) partial;
	}
