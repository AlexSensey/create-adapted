...
public abstract class FlowSource {
	...
	public static class FluidHandler extends FlowSource {
		...
		public void manageSource(Level level, BlockEntity networkBE) {
			...
			if (blockEntity == null)
				return;
			if (blockEntity instanceof HosePulleyBlockEntity hosePulley
				&& HosePulleyBlock.hasPipeTowards(level, hosePulley.getBlockPos(), hosePulley.getBlockState(),
					location.getOppositeFace())) {
				fluidHandlerCache = ICapabilityProvider.of(hosePulley::getFluidHandlerForPipe);
				return;
			}
			if (level instanceof ServerLevel serverLevel && !PonderLevelCompat.isPonderLevel(level)) {
			...
			} else if (PonderLevelCompat.isPonderLevel(level)) {
				fluidHandlerCache = ICapabilityProvider.of(() -> {
