...
public class FluidMesh {
	...
	private static float uvShrinkRatio(TextureAtlasSprite sprite) {
		return 4f / Math.max(sprite.contents().width(), sprite.contents().height());
	}
	...
	public record FluidSurfaceMesh(TextureAtlasSprite texture, float width) implements QuadMesh {
		...
		public void write(MutableVertexList vertexList) {
			...
			float shrink = uvShrinkRatio(texture) * 0.25f * textureScale;
