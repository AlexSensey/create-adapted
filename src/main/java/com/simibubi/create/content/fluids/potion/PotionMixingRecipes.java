...
public class PotionMixingRecipes {
	...
	public static final List<Item> SUPPORTED_CONTAINERS =
	...
	private static List<RecipeHolder<MixingRecipe>> createRecipesImpl(Level level) {
		...
		for (Item container : allowedSupportedContainers) {
			...
			for (PotionBrewing.Mix<Potion> mix : ((PotionBrewingAccessor) potionBrewing).create$getPotionMixes()) {
				FluidStack fromFluid =
					PotionFluidHandler.getFluidFromPotion(new PotionContents(mix.from()), bottleType, 1000);
				FluidStack toFluid = PotionFluidHandler.getFluidFromPotion(new PotionContents(mix.to()), bottleType, 1000);

				mixingRecipes.add(createRecipe("potion_mixing_vanilla_" + recipeIndex++, mix.ingredient(), fromFluid,
					toFluid));
			}
		}
		...
		for (PotionBrewing.Mix<Item> mix : ((PotionBrewingAccessor) potionBrewing).create$getContainerMixes()) {
			...
			for (Reference<Potion> potion : potions) {
				FluidStack fromFluid =
					PotionFluidHandler.getFluidFromPotion(new PotionContents(potion), fromBottleType, 1000);
				FluidStack toFluid = PotionFluidHandler.getFluidFromPotion(new PotionContents(potion), toBottleType,
					1000);
		...
		for (IBrewingRecipe recipe : potionBrewing.getRecipes()) {
			...
			// Ingredient#items only exposes holders in 26.2 and discards the potion
			// data components. The old getItems() returned complete ItemStacks; resolve
			// the display to preserve the exact input potion in generated JEI recipes.
			ItemStack inputStack = input.display()
				.resolveForFirstStack(SlotDisplayContext.fromLevel(level));
			if (inputStack.isEmpty())
				continue;
			for (ItemStack container : supportedContainerStacks) {
				if (!input.test(container))
					continue;
				FluidStack inputFluid = PotionFluidHandler.getFluidFromPotionItem(inputStack);
				FluidStack outputFluid = PotionFluidHandler.getFluidFromPotionItem(output);
				inputFluid.setAmount(1000);
				outputFluid.setAmount(1000);
				mixingRecipes.add(createRecipe("potion_mixing_modded_" + moddedRecipeIndex++, ingredient,
	...
	private static RecipeHolder<MixingRecipe> createRecipe(String id, Ingredient ingredient, FluidStack fromFluid,
		FluidStack toFluid) {
	...
	private static Map<Item, List<MixingRecipe>> sortRecipesByItem(List<RecipeHolder<MixingRecipe>> all) {
		...
		for (RecipeHolder<MixingRecipe> recipe : all) {
			for (Ingredient ingredient : recipe.value()
				.getIngredients()) {
				ingredient.items()
					.forEach(holder -> {
						Item item = holder.value();
						if (processedItems.add(item))
							byItem.computeIfAbsent(item, $ -> new ArrayList<>())
								.add(recipe.value());
					});
			}
