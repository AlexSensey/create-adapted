...
public abstract class ProcessingRecipeBuilder<P extends ProcessingRecipeParams, R extends ProcessingRecipe<?, P>, S extends ProcessingRecipeBuilder<P, R, S>> {
	...
	public S withRegistries(HolderLookup.Provider registries) {
	...
	public S withFluidOutputs(NonNullList<FluidStack> outputs) {
		...
		outputs.forEach(output -> params.fluidResults.add(ProcessingFluidOutput.of(output)));
		return self();
	}
	...
	@SafeVarargs
	public final S require(TagKey<Item>... tags) {
		var lookup = registries.lookupOrThrow(Registries.ITEM);
		if (tags.length == 1)
			return require(Ingredient.of(lookup.getOrThrow(tags[0])));
		Ingredient[] alternatives = Arrays.stream(tags)
			.map(tag -> Ingredient.of(lookup.getOrThrow(tag)))
			.toArray(Ingredient[]::new);
		return require(CompoundIngredient.of(alternatives));
	}
	...
	public S require(TagKey<Fluid> fluidTag, int amount) {
		var lookup = registries.lookupOrThrow(Registries.FLUID);
		return require(new SizedFluidIngredient(FluidIngredient.of(lookup.getOrThrow(fluidTag)), amount));
	}
	...
	public S output(FluidStack fluidStack) {
		params.fluidResults.add(ProcessingFluidOutput.of(fluidStack));
		return self();
	}
