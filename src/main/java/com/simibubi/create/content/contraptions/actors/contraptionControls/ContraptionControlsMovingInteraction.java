...
public class ContraptionControlsMovingInteraction extends MovingInteractionBehaviour {
	...
	private static boolean isDoorFilter(ItemStack filter) {
		return filter.getItem() instanceof BlockItem blockItem
			&& blockItem.getBlock()
				.defaultBlockState()
				.is(BlockTags.DOORS);
	}
	...
	private boolean elevatorInteraction(BlockPos localPos, AbstractContraptionEntity contraptionEntity,
		...
		if (!(ctx.temporaryData instanceof ElevatorFloorSelection)) {
			...
			ContraptionControlsMovement.initFloorSelectionAtCurrentTarget(selection, contraption);
			ctx.temporaryData = selection;
		}
		ElevatorFloorSelection efs = (ElevatorFloorSelection) ctx.temporaryData;
		ContraptionControlsMovement.tickFloorSelection(efs, contraption);
		...
		if (contraptionEntity instanceof ControlledContraptionEntity cce) {
			BlockPos controllerPos = cce.getControllerBlockPos();
			if (controllerPos != null
				&& level.getBlockEntity(controllerPos) instanceof ElevatorPulleyBlockEntity epbe)
				epbe.updateClientTarget(contraption, efs.currentTargetY);
		}
		if (contraption.getBlockEntityClientSide(ctx.localPos) instanceof ContraptionControlsBlockEntity cbe)
			cbe.pressButton();
		return true;
