...
public class PloughMovementBehaviour extends BlockBreakingMovementBehaviour {
	...
	public boolean isActive(MovementContext context) {
		return super.isActive(context)
			&& !VecHelper.isVecPointingTowards(context.relativeMotion,
				context.state.getValue(PloughBlock.FACING)
					.getOpposite());
	}
	...
	private void discardPlayer(MovementContext context) {
	...
	private PloughFakePlayer getPlayer(MovementContext context) {
		if (!(context.world instanceof ServerLevel world))
			return null;
		if (!(context.temporaryData instanceof PloughFakePlayer)) {
			PloughFakePlayer player = new PloughFakePlayer(world);
