...
public class SandPaperItem extends Item implements CustomUseEffectsItem {
	...
	public ItemStack finishUsingItem(ItemStack stack, Level level, LivingEntity entityLiving) {
		...
		if (stack.has(AllDataComponents.SAND_PAPER_POLISHING)) {
			...
			stack.hurtAndBreak(1, entityLiving, slotForHand(entityLiving.getUsedItemHand()));
		}
	...
	public static void spawnParticles(Vec3 location, ItemStack polishedStack, Level world) {
		for (int i = 0; i < 20; i++) {
			...
			world.addParticle(new ItemParticleOption(ParticleTypes.ITEM, polishedStack.getItem()), location.x, location.y,
	...
	public InteractionResult useOn(UseOnContext context) {
		...
		if (newState != null) {
			...
			if (player != null)
				stack.hurtAndBreak(1, player, slotForHand(player.getUsedItemHand()));
			return InteractionResult.SUCCESS;
		}
	...
	public void onUseTick(Level level, LivingEntity entity, ItemStack stack, int remainingUseDuration) {
		// LivingEntity's shouldTriggerItemUseEffects/triggerItemUseEffects hooks were
		// removed in 26.2. Item#onUseTick is now the per-tick use callback.
		triggerUseEffects(stack, entity, 1, entity.getRandom());
	}
