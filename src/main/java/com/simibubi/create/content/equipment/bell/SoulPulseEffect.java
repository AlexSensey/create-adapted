...
public class SoulPulseEffect {
	...
	public static boolean canSpawnSoulAt(Level world, BlockPos at, boolean ignoreLight) {
		...
		double dummyWidth = .2;
		double dummyHeight = .75;
		double halfWidth = dummyWidth / 2;

		return world != null && SpawnPlacementTypes.ON_GROUND.isSpawnPositionOk(world, at, dummy)
			&& (ignoreLight || isDark(world, at))
			&& Streams.stream(world.getBlockCollisions(null,
				new AABB(at.getX() + .5 - halfWidth, at.getY(), at.getZ() + .5 - halfWidth,
					at.getX() + .5 + halfWidth, at.getY() + dummyHeight, at.getZ() + .5 + halfWidth)))
