...
public class AllPotatoProjectileEntityHitActions {
	...
	public record FoodEffects(FoodProperties foodProperty,
		...
		public boolean execute(ItemStack projectile, EntityHitResult ray, Type type) {
			...
			if (entity instanceof LivingEntity livingEntity) {
				// Food side effects moved out of FoodProperties and into Consumable in 26.2.
				// Read them from the fired item so pufferfish, golden apples and modded
				// food ammunition retain the same on-hit behaviour as before the port.
