...
public class PotatoCannonRenderHandler extends ShootableGadgetRenderHandler {
	...
	public void beforeShoot(float nextPitch, Vec3 location, Vec3 motion, ItemStack stack) {
		...
		if (level == null)
			return;
		for (int i = 0; i < 2; i++) {
			Vec3 itemMotion = VecHelper.offsetRandomly(motion.scale(.1f), level.getRandom(), .025f);
			level.addParticle(new ItemParticleOption(ParticleTypes.ITEM, ItemStackTemplate.fromNonEmptyStack(stack)),
				location.x, location.y, location.z,
				itemMotion.x, itemMotion.y, itemMotion.z);
			Vec3 airMotion = VecHelper.offsetRandomly(motion.scale(2), level.getRandom(), .5f);
			level.addParticle(new AirParticleData(1, .25f), location.x, location.y, location.z,
				airMotion.x, airMotion.y, airMotion.z);
		}
	}
