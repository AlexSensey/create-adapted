...
public class ClipboardScreen extends AbstractSimiScreen {
	...
	public void reopenWith(ClipboardContent content) {
		...
		if (readonly)
			editingIndex = -1;
		// A block entity update can arrive while this screen is open. Re-running
		// init() here clears and recreates every widget during GUI extraction,
		// making the page arrows and side buttons visibly disappear for a frame.
		// Their positions do not depend on the clipboard contents; only refresh
		// the page availability after accepting the synchronized content.
		if (forward != null)
	...
	protected void init() {
		...
		forward = new StablePageButton(x + 176, y + 229, true, $ -> changePage(true));
		backward = new StablePageButton(x + 53, y + 229, false, $ -> changePage(false));
	...
	public void tick() {
		...
		if (targetedBlock != null) {
			if (minecraft.player.distanceToSqr(Vec3.atCenterOf(targetedBlock)) > 100) {
	...
	public void extractRenderState(GuiGraphicsExtractor graphics, int mouseX, int mouseY, float partialTicks) {
		...
		// In 26.2 Screen renders its widgets from this superclass call. It must
		// run even when no clipboard row is currently being edited.
	...
	public boolean charTyped(CharacterEvent event) {
		...
		if (editingIndex == -1)
			return false;
		editContext.insertText(event.codepointAsString());
		clearDisplayCache();
		return true;
	}
	...
	private boolean keyPressedWhileEditing(KeyEvent event) {
		...
		if (event.isSelectAll()) {
			...
			case 262:
				if (event.hasControlDown()) {
					editContext.moveByWords(1, event.hasShiftDown());
					return true;
				}
				editContext.moveByChars(1, event.hasShiftDown());
				return true;
			case 263:
				if (event.hasControlDown()) {
					editContext.moveByWords(-1, event.hasShiftDown());
				...
				return true;
			case 264:
				keyDown(event.hasShiftDown());
				return true;
			case 265:
				keyUp(event.hasShiftDown());
				return true;
			case 268:
				keyHome(event.hasShiftDown());
				return true;
			case 269:
				keyEnd(event.hasShiftDown());
				return true;
			default:
				return false;
			}
	...
	private void keyUp(boolean selecting) {
		changeLine(-1, selecting);
	}

	private void keyDown(boolean selecting) {
		changeLine(1, selecting);
	}

	private void changeLine(int pYChange, boolean selecting) {
		int i = editContext.getCursorPos();
		int j = getDisplayCache().changeLine(i, pYChange);
		editContext.setCursorPos(j, selecting);
	}

	private void keyHome(boolean selecting) {
		int i = editContext.getCursorPos();
		int j = getDisplayCache().findLineStart(i);
		editContext.setCursorPos(j, selecting);
	}

	private void keyEnd(boolean selecting) {
		DisplayCache cache = getDisplayCache();
		int i = editContext.getCursorPos();
	...
	public boolean mouseClicked(MouseButtonEvent event, boolean doubleClick) {
		...
		if (pMouseX < left + 50 || pMouseX > left + 220 || pMouseY < top + 30 || pMouseY > top + 230) {
			setFocused(null);
			clearDisplayCache();
			editingIndex = -1;
			return false;
		...
		if (j >= 0) {
			if (j == lastIndex && i - lastClickTime < 250L) {
				...
				editContext.setCursorPos(j, event.hasShiftDown());
			}
	...
	/**
	 * Minecraft 26.2 swaps PageButton to a highlighted GUI sprite while the
	 * cursor is over it. That highlighted sprite is not stable in the current
	 * extracted GUI atlas, so the arrow appears to blink. Keep the original
	 * book arrow sprite and retain the vanilla button interaction and sound.
	 */
	@OnlyIn(Dist.CLIENT)
	static class StablePageButton extends PageButton {
		...
		StablePageButton(int x, int y, boolean forward, OnPress onPress) {
			super(x, y, forward, onPress, true);
			this.forward = forward;
		}

		@Override
		public void extractContents(GuiGraphicsExtractor graphics, int mouseX, int mouseY, float partialTicks) {
			graphics.blitSprite(RenderPipelines.GUI_TEXTURED, forward ? FORWARD : BACKWARD,
				getX(), getY(), 23, 13);
		}
	}
