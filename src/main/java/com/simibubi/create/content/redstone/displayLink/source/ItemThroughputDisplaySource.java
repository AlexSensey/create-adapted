...
public class ItemThroughputDisplaySource extends AccumulatedItemCountDisplaySource {
	...
	protected MutableComponent provideLine(DisplayLinkContext context, DisplayTargetStats stats) {
		...
		double interval = 20 * Math.pow(60, conf.getInt("Interval").orElse(0));
		double rate = conf.getFloat("Rate").orElse(0f) * interval;

		if (rate > 0) {
			long previousTime = conf.getLong("LastReceived").orElse(0L);
			...
			if (diff > 0) {
				...
				int lastAmount = conf.getInt("LastReceivedAmount").orElse(0);
	...
	public void itemReceived(DisplayLinkBlockEntity be, int amount) {
		...
		int poolIndex = conf.getInt("Index").orElse(0) % POOL_SIZE;
		rates.set(poolIndex, FloatTag.valueOf((float) (amount / (double) (gameTime - previousTime))));

		float rate = 0;
		int validIntervals = 0;
		for (int i = 0; i < POOL_SIZE; i++) {
			float pooledRate = rates.getFloatOr(i, -1);
			if (pooledRate >= 0) {
				rate += pooledRate;
				validIntervals++;
			}
