...
public class LinkedControllerItemRenderer extends CustomRenderedItemModelRenderer {
	...
	protected static final Identifier POWERED = Create.asResource("linked_controller_powered");
	protected static final Identifier POWERED_PATH = Create.asResource("item/linked_controller/powered");
	protected static final Identifier BUTTON = Create.asResource("linked_controller_button");
	protected static final Identifier BUTTON_PATH = Create.asResource("item/linked_controller/button");
	...
	protected static void renderNormal(ItemStack stack, CustomRenderedItemModel model,
		PartialItemModelRenderer renderer, ItemDisplayContext transformType, PoseStack ms, int light) {
	...
	protected static void render(ItemStack stack, CustomRenderedItemModel model,
		...
		if (renderType == RenderType.NORMAL) {
			...
			active = false;
			boolean noControllerInMain = mc.player == null || !AllItems.LINKED_CONTROLLER.isIn(mc.player.getMainHandItem());

			if (mc.player != null && (transformType == mainHand || (transformType == offHand && noControllerInMain))) {
			...
			if (mc.player != null && transformType == ItemDisplayContext.GUI) {
		...
		BakedModel powered = getPartial(POWERED, POWERED_PATH);
		renderer.render(active && powered != null ? powered : model.getOriginalModel(), light);
		...
		BakedModel button = getPartial(BUTTON, BUTTON_PATH);
		if (button == null) {
			ms.popPose();
			return;
		}
		...
		if (renderType == RenderType.NORMAL && LinkedControllerClientHandler.MODE == Mode.BIND) {
	...
	protected static void renderButton(PartialItemModelRenderer renderer, PoseStack ms, int light, float pt,
		BakedModel button, float b, int index, boolean renderDepression) {
		ms.pushPose();
		if (renderDepression) {
			float depression = b * buttons.get(index)
				.getValue(pt);
	...
	private static BakedModel getPartial(Identifier primary, Identifier fallback) {
		BakedModel model = CustomRenderedItems.getPartialModel(primary);
		return model != null ? model : CustomRenderedItems.getPartialModel(fallback);
	}
