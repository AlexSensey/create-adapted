...
public class BeltDeployerCallbacks {
	...
	public static void activate(TransportedItemStack transported, TransportedItemStackHandlerBehaviour handler,
		...
		handler.handleProcessingOnItem(transported, process(transported, blockEntity, recipe));
	}

	public static TransportedResult process(TransportedItemStack transported, DeployerBlockEntity blockEntity,
											Recipe<?> recipe) {
		...
		boolean hasRecipeOutput = !collect.isEmpty();
		if (!hasRecipeOutput) {
		...
		if (!keepHeld) {
			if (heldItem.getMaxDamage() > 0) {
				...
				ItemStackTemplate remainder = heldItem.getItem()
		...
		// The client renderer reads the synchronized cache, while recipe processing
		// mutates the fake player's live hand. Keep both views in lockstep.
		blockEntity.heldItem = blockEntity.player.getMainHandItem()
			.copy();
