...
public class ArmVisual extends SingleAxisRotatingVisual<ArmBlockEntity> implements SimpleDynamicVisual {
	...
	private void updateAngles(float baseAngle, float lowerArmAngle, float upperArmAngle, float headAngle, int color) {
		...
		ArmRenderer.transformBase(msr.unwrap(), baseAngle);
		base.setTransform(poseStack)
			.setChanged();

		ArmRenderer.transformLowerArm(msr.unwrap(), lowerArmAngle);
		lowerBody.setTransform(poseStack)
			.colorRgb(color)
			.setChanged();

		ArmRenderer.transformUpperArm(msr.unwrap(), upperArmAngle);
		upperBody.setTransform(poseStack)
			.colorRgb(color)
			.setChanged();

		ArmRenderer.transformHead(msr.unwrap(), headAngle);
		...
		for (int index : Iterate.zeroAndOne) {
			poseStack.pushPose();
			int flip = index * 2 - 1;
			ArmRenderer.transformClawHalf(msr.unwrap(), hasItem, isBlockItem, flip);
			clawGrips.get(index)
