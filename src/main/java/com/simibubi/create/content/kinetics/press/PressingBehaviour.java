...
public class PressingBehaviour extends BeltProcessingBehaviour {
	...
	public void read(CompoundTag compound, HolderLookup.Provider registries, boolean clientPacket) {
		...
		if (clientPacket) {
			for (Tag tag : compound.getListOrEmpty("ParticleItems")) {
				ItemStack.OPTIONAL_CODEC.decode(registries.createSerializationContext(NbtOps.INSTANCE), tag)
					.result()
					.map(result -> result.getFirst())
					.filter(stack -> !stack.isEmpty())
					.ifPresent(particleItems::add);
			}
			spawnParticles();
		}
	}
	...
	public void write(CompoundTag compound, HolderLookup.Provider registries, boolean clientPacket) {
		...
		if (clientPacket) {
			ListTag particleItemTags = new ListTag();
			for (ItemStack stack : particleItems) {
				ItemStack.OPTIONAL_CODEC.encodeStart(registries.createSerializationContext(NbtOps.INSTANCE), stack)
					...
					.ifPresent(particleItemTags::add);
			}
			compound.put("ParticleItems", particleItemTags);
			particleItems.clear();
		}
	}
	...
	public void makePressingParticleEffect(Vec3 pos, ItemStack stack, int amount) {
		...
		for (int i = 0; i < amount; i++) {
			...
			level.addParticle(new ItemParticleOption(ParticleTypes.ITEM, stack.getItem()), pos.x, pos.y - .25f, pos.z,
	...
	public void makeCompactingParticleEffect(Vec3 pos, ItemStack stack) {
		...
		for (int i = 0; i < 20; i++) {
			Vec3 motion = VecHelper.offsetRandomly(Vec3.ZERO, level.getRandom(), .175f)
				.multiply(1, 0, 1);
			level.addParticle(new ItemParticleOption(ParticleTypes.ITEM, stack.getItem()), pos.x, pos.y, pos.z, motion.x,
