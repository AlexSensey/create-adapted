...
public class BeltModel extends CreateBakedModelWrapper<BakedModel> {
	...
	public TextureAtlasSprite getParticleIcon(ModelData data) {
		if (!data.has(BeltModelData.CASING_PROPERTY))
			return super.getParticleIcon(data);
		CasingType type = data.get(BeltModelData.CASING_PROPERTY);
	...
	public List<BakedQuad> getQuads(BlockState state, Direction side, RandomSource rand, ModelData extraData,
		...
		if (!extraData.has(BeltModelData.CASING_PROPERTY))
			return quads;

		boolean cover = extraData.has(BeltModelData.COVER_PROPERTY) && Boolean.TRUE.equals(extraData.get(BeltModelData.COVER_PROPERTY));
		CasingType type = extraData.get(BeltModelData.CASING_PROPERTY);
		...
		if (cover) {
			boolean alongX = state.getValue(BeltBlock.HORIZONTAL_FACING)
				.getAxis() == Axis.X;
			BakedModel coverModel = (BakedModel)
