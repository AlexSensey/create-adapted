	...
	public static void manualApplicationRecipesApplyInWorld(PlayerInteractEvent.RightClickBlock event) {
		...
		Optional<RecipeHolder<Recipe<RecipeWrapper>>> foundRecipe = serverLevel.recipeAccess()
			...
			.map(holder -> {
				...
				RecipeHolder<Recipe<RecipeWrapper>> typedHolder =
					(RecipeHolder<Recipe<RecipeWrapper>>) (RecipeHolder<?>) holder;
				return typedHolder;
			})
			.filter(holder -> {
				if (!(holder.value() instanceof ManualApplicationRecipe recipe))
					return false;
				return recipe.testBlock(blockState) && recipe.ingredients.get(1)
					.test(heldItem);
			})
			.findFirst();
