...
public class ChainConveyorInteractionHandler {
	...
	public static void clientTick() {
		...
		boolean isWrench = AllItems.WRENCH.isIn(mainHandItem);
		boolean isChain = mainHandItem.is(Items.IRON_CHAIN);
		...
		for (Entry<BlockPos, List<ChainConveyorShape>> entry : loadedChains.get(mc.level)
			.asMap()
			.entrySet()) {
	...
	private static boolean isActive() {
		...
		if (mc.player == null)
			return false;
		ItemStack mainHandItem = mc.player.getMainHandItem();
		return mainHandItem.is(Items.IRON_CHAIN) || AllItems.WRENCH.isIn(mainHandItem) || AllBlocks.PACKAGE_FROGPORT.isIn(mainHandItem)
	...
	public static boolean onUse() {
		...
		if (AllItems.WRENCH.isIn(mainHandItem)) {
			...
			if (selectedConnection != null)
				ClientNetworkHelper.INSTANCE.sendToServer(new ChainConveyorConnectionPacket(selectedLift,
					selectedLift.offset(selectedConnection), mainHandItem, false));
			return true;
		}
		...
		if (PackageItem.isPackage(mainHandItem)) {
			ClientNetworkHelper.INSTANCE.sendToServer(new ChainPackageInteractionPacket(selectedLift, selectedConnection,
				selectedChainPosition, false));
			return true;
		}
