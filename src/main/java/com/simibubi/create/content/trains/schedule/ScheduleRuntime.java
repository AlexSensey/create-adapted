...
public class ScheduleRuntime {
	...
	public CompoundTag write(HolderLookup.Provider registries) {
		...
		tag.putIntArray("ConditionProgress", toIntArray(conditionProgress));
		tag.put("ConditionContext", NBTHelper.writeCompoundList(conditionContext, CompoundTag::copy));
		tag.putIntArray("TransitTimes", toIntArray(predictionTicks));
		return tag;
	}
	...
	public void read(HolderLookup.Provider registries, CompoundTag tag) {
		...
		state = readEnum(tag, "State", State.class, State.PRE_TRANSIT);
		for (int i : tag.getIntArray("ConditionProgress").orElse(new int[0]))
			conditionProgress.add(i);
		for (Tag t : tag.getListOrEmpty("ConditionContext"))
			if (t instanceof CompoundTag c)
				conditionContext.add(c);
