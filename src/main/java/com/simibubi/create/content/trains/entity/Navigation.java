...
public class Navigation {
	...
	public CompoundTag write(DimensionPalette dimensions) {
		...
		tag.putString("Destination", destination.id.toString());
		...
		if (waitingForSignal == null)
			return tag;
		tag.putString("BlockingSignal", waitingForSignal.getFirst().toString());
	...
	public void read(CompoundTag tag, TrackGraph graph, DimensionPalette dimensions) {
		destination = graph != null && tag.contains("Destination")
			? graph.getPoint(EdgePointType.STATION, readUuid(tag, "Destination"))
			: null;
		...
		currentPath.clear();
		for (Tag t : tag.getListOrEmpty("Path"))
			if (t instanceof CompoundTag c)
				currentPath.add(Couple
		...
		waitingForSignal = tag.contains("BlockingSignal")
			? Pair.of(readUuid(tag, "BlockingSignal"), tag.getBooleanOr("BlockingSignalSide", false))
			: null;
		if (waitingForSignal == null)
			return;
