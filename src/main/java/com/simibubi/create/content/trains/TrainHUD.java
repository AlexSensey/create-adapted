...
public class TrainHUD {
	...
	private static void renderOverlay(GuiGraphicsExtractor guiGraphics, DeltaTracker deltaTracker) {
		...
		poseStack.translate(guiGraphics.guiWidth() / 2f - 91, guiGraphics.guiHeight() - 29);
		...
		guiGraphics.blit(RenderPipelines.GUI_TEXTURED, AllGuiTextures.TRAIN_HUD_SPEED.location, 0, 0,
		...
		if (promptSize > 1) {
			...
			guiGraphics.blit(RenderPipelines.GUI_TEXTURED, AllGuiTextures.TRAIN_PROMPT.location, 0, 0,
		...
		guiGraphics.blit(RenderPipelines.GUI_TEXTURED, AllGuiTextures.TRAIN_HUD_THROTTLE.location, invW, 0,
			AllGuiTextures.TRAIN_HUD_THROTTLE.getStartX() + invW, AllGuiTextures.TRAIN_HUD_THROTTLE.getStartY(), w, h,
			256, 256);
		...
		int directionFrame = Math.floorMod(-Math.round(snappedAngle / snapSize), 16);

		poseStack.translate(91, -9);
		poseStack.scale(0.925f, 0.925f);
		guiGraphics.blit(RenderPipelines.GUI_TEXTURED, CatnipGuiTextures.PLACEMENT_INDICATOR_SHEET.getId(), -8, -8, 0,
			directionFrame * 16, 16, 16, 16, 256);
