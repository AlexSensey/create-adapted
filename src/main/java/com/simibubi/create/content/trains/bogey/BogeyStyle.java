...
public class BogeyStyle {
	...
	public void submit(BogeySize size, float partialTick, PoseStack poseStack, SubmitNodeCollector collector,
		int light, int overlay, float wheelAngle, @Nullable CompoundTag bogeyData, boolean inContraption) {
		if (bogeyData == null)
			bogeyData = new CompoundTag();
		...
		if (renderer != null) {
			poseStack.pushPose();
			poseStack.translate(0, -1.5 - 1 / 128f, 0);
			renderer.renderer.submit(bogeyData, wheelAngle, partialTick, poseStack, collector, light, overlay,
				inContraption);
			poseStack.popPose();
		}
	}
