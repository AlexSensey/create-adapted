...
public class TrackGraphHelper {
	...
	public static TrackGraphLocation getBezierGraphLocationAt(Level level, BlockPos pos, AxisDirection targetDirection,
		...
		TrackNodeLocation startLoc = new TrackNodeLocation(bc.starts.getFirst()).in(level);
		if (bc.smoothing != null)
			startLoc.yOffsetPixels = bc.smoothing.getFirst();

		for (TrackGraph graph : Create.RAILWAYS.sided(level).trackNetworks.values()) {
			TrackNode startNode = graph.locateNode(startLoc);
			TrackNode targetNode = graph.locateNode(targetLoc);
			if (startNode == null || targetNode == null)
				continue;

			TrackEdge edge = graph.getConnectionsFrom(startNode)
				.get(targetNode);
			boolean reversed = false;
			if (edge == null) {
				edge = graph.getConnectionsFrom(targetNode)
					.get(startNode);
				reversed = edge != null;
			}
			if (edge == null)
			...
			TrackGraphLocation graphLocation = new TrackGraphLocation();
			graphLocation.graph = graph;
			graphLocation.edge = Couple.create(startLoc, targetLoc);
			graphLocation.position = (targetBezier.segment() + 1) / 2f;
			if (reversed) {
				graphLocation.edge = graphLocation.edge.swap();
				graphLocation.position = edge.getLength() - graphLocation.position;
