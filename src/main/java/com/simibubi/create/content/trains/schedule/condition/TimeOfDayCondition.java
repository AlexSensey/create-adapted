...
public class TimeOfDayCondition extends ScheduleWaitCondition {
	...
	public boolean tickCompletion(Level level, Train train, CompoundTag context) {
		...
		int dayTime = (int) (dayTime(level) % getRotation());
		int targetTicks =
	...
	public boolean renderSpecialIcon(GuiGraphicsExtractor graphics, int x, int y) {
		...
		Identifier location = Identifier.withDefaultNamespace("textures/item/clock_"
			+ twoDigits(Mth.clamp((int) (progress * 64), 0, 63)) + ".png");
	...
	public MutableComponent getWaitingStatus(Level level, Train train, CompoundTag tag) {
		...
		int departureTime = (int) (dayTime(level) + diff) % 24000;
	...
	private long dayTime(Level level) {
