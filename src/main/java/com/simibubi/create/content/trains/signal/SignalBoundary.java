	...
	public void read(CompoundTag nbt, HolderLookup.Provider registries, boolean migration, DimensionPalette dimensions) {
		...
		for (int i = 1; i <= 2; i++)
			if (nbt.contains("Tiles" + i)) {
				boolean first = i == 1;
				for (Tag t : nbt.getListOrEmpty("Tiles" + i))
					if (t instanceof CompoundTag c)
						blockEntities.get(first)
							.put(readBlockPos(c.getCompoundOrEmpty("Pos")), c.getBooleanOr("Power", false));
			}

		for (int i = 1; i <= 2; i++)
			if (nbt.contains("Group" + i))
				groups.set(i == 1, readUuid(nbt, "Group" + i));
		for (int i = 1; i <= 2; i++)
			sidesToUpdate.set(i == 1, nbt.contains("Update" + i));
		for (int i = 1; i <= 2; i++)
			types.set(i == 1, readEnum(nbt, "Type" + i, SignalType.class, SignalType.ENTRY_SIGNAL));
		for (int i = 1; i <= 2; i++)
			cachedStates.set(i == 1, readEnum(nbt, "State" + i, SignalState.class, SignalState.INVALID));
	}
	...
	public void write(CompoundTag nbt, HolderLookup.Provider registries, DimensionPalette dimensions) {
		...
		for (int i = 1; i <= 2; i++)
			if (groups.get(i == 1) != null)
				nbt.putString("Group" + i, groups.get(i == 1).toString());
		for (int i = 1; i <= 2; i++)
			if (sidesToUpdate.get(i == 1))
				nbt.putBoolean("Update" + i, true);
		for (int i = 1; i <= 2; i++)
			writeEnum(nbt, "Type" + i, types.get(i == 1));
		for (int i = 1; i <= 2; i++)
			writeEnum(nbt, "State" + i, cachedStates.get(i == 1));
	}
