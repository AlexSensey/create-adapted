...
public class CarriageContraptionEntity extends OrientedContraptionEntity {
	...
	private static final EntityDataAccessor<String> TRACK_GRAPH =
		SynchedEntityData.defineId(CarriageContraptionEntity.class, EntityDataSerializers.STRING);
	...
	private final InterpolationHandler noopVanillaPositionInterpolation;
	...
	public CarriageContraptionEntity(EntityType<?> type, Level world) {
		super(type, world);
		noopVanillaPositionInterpolation = new InterpolationHandler(this) {
			@Override
	...
	public InterpolationHandler getInterpolation() {
		return noopVanillaPositionInterpolation;
	}
	...
	protected void tickContraption() {
		...
		if (!level().isClientSide()) {
			...
			entityData.set(TRACK_GRAPH, carriage.train.graph == null ? "" : carriage.train.graph.id.toString());
	...
	protected void writeAdditional(CompoundTag compound, HolderLookup.Provider registries, boolean spawnPacket) {
		...
		if (trainId != null)
			compound.putString("TrainId", trainId.toString());
	...
	protected void readAdditional(CompoundTag compound, boolean spawnPacket) {
		...
		trainId = parseUuid(compound.getStringOr("TrainId", ""));
	...
	private void sendPrompt(Player player, Component component, boolean shadow) {
	...
	private void cleanUpApproachStationMessage(Player player) {
		if (!stationMessage)
			return;
		sendPrompt(player, CommonComponents.EMPTY, false);
		stationMessage = false;
	}
	...
	private void updateTrackGraph() {
		if (carriage == null)
			return;
		UUID graphId = parseUuid(entityData.get(TRACK_GRAPH));
		if (graphId == null) {
		...
		TrackGraph graph = CreateClient.RAILWAYS.sided(level()).trackNetworks.get(graphId);
		if (graph == null)
			return;
	...
	public void setCarriage(Carriage carriage) {
		...
		if (carriage.train.graph != null)
			entityData.set(TRACK_GRAPH, carriage.train.graph.id.toString());
