...
public class DimensionPalette {
	...
	public void send(FriendlyByteBuf buffer) {
		...
		gatheredDims.forEach(buffer::writeResourceKey);
	}
	...
	public static DimensionPalette receive(FriendlyByteBuf buffer) {
		...
		for (int i = 0; i < length; i++)
			palette.gatheredDims.add(buffer.readResourceKey(Registries.DIMENSION));
		return palette;
	}
	...
	public static DimensionPalette read(CompoundTag tag) {
		DimensionPalette palette = new DimensionPalette();
		for (Tag t : tag.getListOrEmpty("DimensionPalette"))
			if (t instanceof CompoundTag c)
				palette.gatheredDims.add(ResourceKey.create(Registries.DIMENSION, Identifier.parse(c.getStringOr("Id", "minecraft:overworld"))));
		return palette;
	}
