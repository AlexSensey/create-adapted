...
public class AllIcons implements ScreenElement {
	...
	public void render(GuiGraphicsExtractor graphics, int x, int y) {
		render(graphics, x, y, 0xFFFFFFFF);
	}

	public void render(GuiGraphicsExtractor graphics, int x, int y, int color) {
		graphics.blit(net.minecraft.client.renderer.RenderPipelines.GUI_TEXTURED, ICON_ATLAS, x, y, iconX, iconY, 16, 16,
			ICON_ATLAS_SIZE, ICON_ATLAS_SIZE, color);
	}

	public void render(PoseStack ms, MultiBufferSource buffer, int color) {
		VertexConsumer builder = buffer.getBuffer(RenderTypes.textSeeThrough(ICON_ATLAS));
		Matrix4f matrix = ms.last()
			.pose();
		render(builder, matrix, color);
	}

	public void render(PoseStack.Pose pose, VertexConsumer builder, int color) {
		render(builder, pose.pose(), color);
	}

	public void renderDoubleSided(PoseStack.Pose pose, VertexConsumer builder, int color) {
		Matrix4f matrix = pose.pose();
		render(builder, matrix, color, false);
		render(builder, matrix, color, true);
	}

	private void render(VertexConsumer builder, Matrix4f matrix, int color) {
		render(builder, matrix, color, false);
	}

	private void render(VertexConsumer builder, Matrix4f matrix, int color, boolean reversed) {
