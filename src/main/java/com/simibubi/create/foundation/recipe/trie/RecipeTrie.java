	...
	public static class Builder<R extends Recipe<?>> {
		...
		private <R1 extends R> AbstractRecipe<R1> createRecipe(R1 recipe) {
			...
			if (recipe instanceof ProcessingRecipe<?, ?> processingRecipe) {
				for (Ingredient ingredient : processingRecipe.getIngredients()) {
					...
					Set<AbstractVariant> variants = new HashSet<>();
					ingredient.items()
						.forEach(holder -> variants.add(getOrAssignVariant(holder.value())));
					ingredients.add(
						variants.isEmpty() ? AbstractIngredient.Universal.INSTANCE : new AbstractIngredient(variants));
				}

				if (recipe instanceof BasinRecipe basinRecipe)
					for (SizedFluidIngredient ingredient : basinRecipe.getFluidIngredients()) {
						if (ingredient.amount() == 0 || !ingredient.ingredient()
							.isSimple()) {
							ingredients.add(AbstractIngredient.Universal.INSTANCE);
							continue;
						}
