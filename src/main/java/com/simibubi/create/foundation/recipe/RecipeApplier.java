	...
	public static List<ItemStack> applyRecipeOn(Level level, ItemStack stackIn, Recipe<?> recipe, boolean returnProcessingRemainder) {
		...
		if (recipe instanceof ProcessingRecipe<?, ?> processingRecipe) {
			...
			for (int i = 0; i < stackIn.getCount(); i++) {
				List<ProcessingOutput> outputs = processingRecipe instanceof ManualApplicationRecipe manualApplicationRecipe
					? manualApplicationRecipe.getRollableResults()
					: processingRecipe.getRollableResults();
				for (ItemStack stack : processingRecipe.rollResults(outputs, level.getRandom()))
					ItemHelper.addToList(stack, stacks);
			}
		} else {
			stacks = ItemHelper.multipliedOutput(stackIn, ItemStack.EMPTY);
		}
