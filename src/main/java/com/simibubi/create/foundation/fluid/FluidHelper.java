...
public class FluidHelper {
	...
	public static boolean tryEmptyItemIntoBE(Level worldIn, Player player, InteractionHand handIn, ItemStack heldItem,
		...
		ResourceHandler<FluidResource> resourceHandler = getBlockFluidResourceHandler(worldIn, be);
		IFluidHandler capability = resourceHandler == null ? null : new ResourceHandlerFluidAdapter(resourceHandler);
		FluidStack fluidStack = emptyingResult.getFirst();

		if (fluidStack.isEmpty())
			return tryInteractWithHeldFluidItem(worldIn, player, handIn, be, resourceHandler);
	...
	private static boolean tryInteractWithHeldFluidItem(Level world, Player player, InteractionHand handIn,
		SmartBlockEntity be, @Nullable ResourceHandler<FluidResource> resourceHandler) {
		if (resourceHandler == null)
			return false;
		if (world.isClientSide())
			return true;
		boolean success = net.neoforged.neoforge.transfer.fluid.FluidUtil.interactWithFluidHandler(player, handIn,
			be.getBlockPos(), resourceHandler, null);
		if (success)
			be.notifyUpdate();
		return success;
	}
	...
	public static boolean tryFillItemFromBE(Level world, Player player, InteractionHand handIn, ItemStack heldItem,
		SmartBlockEntity be) {
		ResourceHandler<FluidResource> resourceHandler = getBlockFluidResourceHandler(world, be);
		if (!GenericItemFilling.canItemBeFilled(world, heldItem))
			return tryInteractWithHeldFluidItem(world, player, handIn, be, resourceHandler);
	...
	@Nullable
	private static IFluidHandler getBlockFluidHandler(Level world, SmartBlockEntity be) {
		ResourceHandler<FluidResource> resourceHandler = getBlockFluidResourceHandler(world, be);
		if (resourceHandler == null)
			return null;
		return new ResourceHandlerFluidAdapter(resourceHandler);
	}

	@Nullable
	private static ResourceHandler<FluidResource> getBlockFluidResourceHandler(Level world, SmartBlockEntity be) {
		return world.getCapability(Capabilities.Fluid.BLOCK, be.getBlockPos(), null);
	}
