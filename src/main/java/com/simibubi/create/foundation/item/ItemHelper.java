...
public class ItemHelper {
	...
	public static <T extends IBE<? extends BlockEntity>> int calcRedstoneFromBlockEntity(T ibe, Level level, BlockPos pos) {
		if (ibe.getBlockEntityOptional(level, pos).isEmpty())
			return 0;
		return calcRedstoneFromInventory(level.getCapability(Capabilities.Item.BLOCK, pos, null));
	}
	...
	public static int calcRedstoneFromInventory(@Nullable ResourceHandler<ItemResource> inv) {
		...
		for (int slot = 0; slot < inv.size(); slot++) {
			...
			int slotLimit = inv.getCapacityAsInt(slot, resource);
			if (slotLimit == 0) {
				totalSlots--;
				continue;
			}
			int amount = inv.getAmountAsInt(slot);
			if (amount > 0 && !resource.isEmpty()) {
				fullness += (float) amount / Math.min(slotLimit, resource.getMaxStackSize());
				occupiedSlots++;
			}
		}
		...
		if (totalSlots == 0)
			return 0;
		return Mth.floor(fullness / totalSlots * 14) + (occupiedSlots > 0 ? 1 : 0);
	}
