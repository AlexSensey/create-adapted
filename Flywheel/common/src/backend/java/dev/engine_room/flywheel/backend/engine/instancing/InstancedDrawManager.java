...
public class InstancedDrawManager extends DrawManager<InstancedInstancer<?>> {
	...
	public void render(LightStorage lightStorage, EnvironmentStorage environmentStorage) {
		...
		if (needSort) {
			...
			// Treat ORDER_INDEPENDENT as ordinary translucency.  The legacy OIT
			// framebuffer chain cannot safely run inside Minecraft 26's render graph.
			draws.addAll(allDraws);
