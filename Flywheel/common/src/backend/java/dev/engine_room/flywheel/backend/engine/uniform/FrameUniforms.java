...
public final class FrameUniforms extends UniformWriter {
	...
	public static void onClientTick() {
		clientTicks = (clientTicks + 1) % 1_728_000;
	}
	...
	public static void update(RenderContext context) {
		...
		Vec3 cameraPos = context.cameraPosition();
		...
		ptr = writeFloat(ptr, depthFar());

		ptr = writeTime(ptr, context);

		ptr = writeCameraIn(ptr, context.level(), camera, cameraPos);
	...
	private static long writeTime(long ptr, RenderContext context) {
		// Match the old LevelRenderer clock. A local clock stays smooth when the
		// server synchronizes or changes the world's game time.
	...
	private static long writeCameraIn(long ptr, Level level, Camera camera, Vec3 cameraPos) {
	...
	private static long writeCullData(long ptr) {
		...
		ptr = writeFloat(ptr, depthFar()); // zFar
		...
		ptr = writeInt(ptr, RenderSystem.getDevice().getDeviceInfo().isZZeroToOne() ? 1 : 0);

		return ptr;
	}

	private static float depthFar() {
