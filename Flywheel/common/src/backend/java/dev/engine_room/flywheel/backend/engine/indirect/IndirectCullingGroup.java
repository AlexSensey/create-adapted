	...
	private void sortDraws() {
		...
		for (int start = 0, i = 0; i < indirectDraws.size(); i++) {
			...
			if (i == indirectDraws.size() - 1 || incompatibleDraws(draw1, indirectDraws.get(i + 1))) {
				// Fall back from ORDER_INDEPENDENT to the normal translucent path.
				// The old OIT path binds its own raw OpenGL framebuffers while Minecraft
				// 26 is executing a render-graph pass, corrupting later passes such as
				// translucent water and the world-unload screen.
