...
public class OitFramebuffer {
	...
	public OitFramebuffer(OitPrograms programs) {
		...
		if (GlCompat.SUPPORTS_DSA) {
			...
			outputFbo = GL46.glCreateFramebuffers();
		} else {
			vao = GL32.glGenVertexArrays();
			outputFbo = GL32.glGenFramebuffers();
		}
	}
	...
	public void prepare() {
		...
		GL32.glBindTexture(GL32.GL_TEXTURE_2D,
			((GlTexture) NoiseTextures.BLUE_NOISE.getTexture()).glId());

		GL32.glBindFramebuffer(GL32.GL_FRAMEBUFFER, fbo);
		GL32.glFramebufferTexture(GL32.GL_FRAMEBUFFER, GL32.GL_DEPTH_ATTACHMENT,
			((GlTexture) renderTarget.getDepthTexture()).glId(), 0);
	}
	...
	public void depthRange() {
		...
		GL32.glBlendFunc(GL32.GL_ONE, GL32.GL_ONE);
		GL32.glBlendEquation(GL32.GL_MAX);

		var far = Minecraft.getInstance().options.getEffectiveRenderDistance() * 16.0F;
	...
	public void renderDepthFromTransmittance() {
		...
		GL32.glColorMask(false, false, false, false);
	...
	public void composite() {
		RenderTarget target = Minecraft.getInstance().gameRenderer.gameRenderState().useShaderTransparency()
			? Minecraft.getInstance().levelRenderer.itemEntityTarget()
		...
		bindOutput(Minecraft.getInstance().gameRenderer.mainRenderTarget());
	}

	public void delete() {
		deleteTextures();
		GL32.glDeleteVertexArrays(vao);
		GL32.glDeleteFramebuffers(outputFbo);
	}
	...
	private void bindOutput(RenderTarget target) {
		int color = ((GlTexture) target.getColorTexture()).glId();
		int depth = target.getDepthTexture() == null ? 0 : ((GlTexture) target.getDepthTexture()).glId();
		if (GlCompat.SUPPORTS_DSA) {
			GL46.glNamedFramebufferTexture(outputFbo, GL32.GL_COLOR_ATTACHMENT0, color, 0);
			GL46.glNamedFramebufferTexture(outputFbo, GL32.GL_DEPTH_ATTACHMENT, depth, 0);
		} else {
			GL32.glBindFramebuffer(GL32.GL_FRAMEBUFFER, outputFbo);
