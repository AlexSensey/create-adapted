...
public final class PonderTooltipHandler {
	...
	private static void openPonder(Item item) {
		Screen currentScreen = ((MinecraftAccessor) Minecraft.getInstance()).catnip$getGui().screen();
	...
	private static Item determineHoveredItem() {
		if (((MinecraftAccessor) Minecraft.getInstance()).catnip$getGui().screen() == null)
			return Items.AIR;
	...
	private static boolean isCurrentPonderSubject(Item item) {
		if (((MinecraftAccessor) Minecraft.getInstance()).catnip$getGui().screen() instanceof PonderUI ponder) {
